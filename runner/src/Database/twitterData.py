...
def tweetLagFox():
    ...
    tweets = db.Twitter_data.find({'isRetweet': 'f', 'similarShowsBeforeTweet': {"$exists": False}})
    numDocuments = (tweets.count())
    print(numDocuments)
    currentDoc = 0
    ...
    for tweet in tweets:
        try:
            ...
            for episode in compare_episodes:
                ...
                ySet = tokenize(text)
                commonGrams = get_similarity(tweet['bagOfWords'],ySet)
                if(commonGrams != None):
                    similarities.append((episode['title'],commonGrams))
...
def tweetLeadFox():
    ...
    tweets = db.Twitter_data.find({'isRetweet': 'f', 'similarShowsAfterTweet': {"$exists": False}})
    numDocuments = (tweets.count())
    print(numDocuments)
    currentDoc = 0
...
def similarShowAnalysis():
    showFrequencyMap = {}
    tweets = db.Twitter_data.find({'isRetweet': 'f', 'similarShowsBeforeTweet': {"$ne": []}})
    for tweet in tweets:
        show = tweet['similarShowsBeforeTweet'][0][0]
        show = show.split(': FBC')
        if(len(show[0]) > 35):
            show[0] = show[0][:35]
        if(show[0] in showFrequencyMap):
            showFrequencyMap[show[0]] +=1
        else:
            showFrequencyMap[show[0]] =1
    # print(showFrequencyMap)
    plt.xticks(rotation='vertical')
    plt.yticks(rotation='vertical')
    plt.bar(list(showFrequencyMap.keys()), showFrequencyMap.values(), color='g')
    # plt.tight_layout()
    plt.subplots_adjust(bottom=0.6)
    plt.savefig('similarShowsBeforeTweet.png')
